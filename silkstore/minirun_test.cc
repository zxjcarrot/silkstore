...

    {
...

        {
            ...
            new_entry.ForEachMiniRunIndexEntry([&i](const MiniRunIndexEntry & e, uint32_t run_no) -> bool{
                ASSERT_EQ(run_no, i);
                ASSERT_EQ(e.GetSegmentNumber(), i);
                ASSERT_EQ(e.GetRunNumberWithinSegment(), i);
        ...
        ASSERT_EQ(new_entry.GetNumMiniRuns(), eNum);
        {
            for (int i = 0; i < eNum; ++i) {
                ...
                ASSERT_EQ(entries[i].GetRunNumberWithinSegment(), eNum * 2);
...

        {
            for (int i = 0; i < eNum; ++i) {
                ...
                if (i + range_size < eNum) {
                    ASSERT_EQ(entries[i].GetRunNumberWithinSegment(), i + range_size);
